...
erpnext.taxes_and_totals = erpnext.payments.extend({
	...
	get_current_tax_amount: function(item, tax, item_tax_map) {
		...
		if(["On Previous Row Amount", "On Previous Row Total","On Pro Row Amount"].includes(tax.charge_type)) {
			if (tax.idx === 1) {
				frappe.throw(
					__("Cannot select charge type as 'On Previous Row Amount' or 'On Previous Row Total' or 'On Pro Row Amount' for first row"));
			}
			if (!tax.row_id) {
		...
		else if(tax.charge_type == "On Pro Row Amount") // added
		{
			current_tax_amount = (tax_rate / 100.0) *
				this.frm.doc["taxes"][cint(tax.row_id) - 2].grand_total_for_current_item;
		}
		else if (tax.charge_type == "On Item Quantity")
		{
